...
transformed parameters{
  ...
  row_vector<lower=0, upper=1>[k] b;
  lambda = g_coef(u ,d , J);
  a = (1+u)/2;
  b = 2*d;
  for (i in 1:(N-J)){
...
ret <- as.vector(na.omit(ret))
ret <- ret[ret!=0]

pdf(file = 'plot/sp500.pdf')
plot(ret, main = "Daily return of S&P 500", ylab = 'r', type = 'l', cex.main = 2, cex.lab = 1.5)
...
plot(ret^2, main = "Squared daily return of S&P 500", ylab = TeX(r'($r^2$)'), type = 'l', cex.main = 2, cex.lab = 1.5)
...
plot(log(ret^2), main = "Log of squared daily return of S&P 500", ylab = TeX(r'($y$)'), type = 'l', cex.main = 2, cex.lab = 1.5)
...
fit3 = sampling(mod, data = test_data, iter = 10000, warmup = 5000,chains = 1)

MSE1 = mean((apply(extract(fit3,'h')$h,2,mean) -1.27 - log(ret[31:length(ret)]^2))^2)
...
fit4 = sampling(mod, data = test_data, iter = 10000, warmup = 5000,chains = 1)
MSE2 = mean((apply(extract(fit4,'h')$h,2,mean) -1.27 - log(ret[31:length(ret)]^2))^2)
...
fit5 = sampling(mod2, data = test_data, iter = 10000, warmup = 5000,chains = 1)

MSE3 = mean((apply(extract(fit5,'h')$h,2,mean) -1.27 - log(ret[31:length(ret)]^2))^2)